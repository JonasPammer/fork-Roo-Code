...
export class AnthropicHandler extends BaseProvider implements SingleCompletionHandler {
	...
	/**
	 * Hook for subclasses to wrap API calls with custom logic (e.g., OAuth token refresh).
	 * Default implementation just executes the function directly.
	 */
	protected async callApiWithRetry<T>(fn: () => Promise<T>): Promise<T> {
		return fn()
	}
	...
	async *createMessage(
		...
		switch (modelId) {
			...
			case "claude-3-haiku-20240307": {
				...
				stream = await this.callApiWithRetry(() =>
	...
	async completePrompt(prompt: string) {
		...
		const message = await this.callApiWithRetry(() =>
			this.client.messages.create({
				model,
